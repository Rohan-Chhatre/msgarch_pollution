...
ma_multi <- read.csv("/Users/rohan/Documents/Rohan/Fall 25/STATINF1/data/MA_multi.csv")
ma_multi$date <- as.Date(ma_multi$date)

pollutant='PM25'
fit_pollutant_msgarch_ma <- function(ma_multi, pollutant, k) {
  ...
  acf_dir   <- "/Users/rohan/Documents/Rohan/Fall 25/STATINF1/plots/acf_residuals_MA"
  pacf_dir  <- "/Users/rohan/Documents/Rohan/Fall 25/STATINF1/plots/pacf_residuals_MA"
  kpss_dir  <- "/Users/rohan/Documents/Rohan/Fall 25/STATINF1/plots/kpss_residuals_MA"
  ...
  acf_file <- file.path(
    acf_dir,
    paste0("ACF_residuals_", pollutant, "_MA.png")
  )
  png(acf_file, width = 1200, height = 800)
  acf(
    y_diag,
    main = paste0("ACF of ARIMA residuals - ", pollutant, " (MA)")
  )
  dev.off()
  ...
  pacf_file <- file.path(
    pacf_dir,
    paste0("PACF_residuals_", pollutant, "_MA.png")
  )
  png(pacf_file, width = 1200, height = 800)
  pacf(
    y_diag,
    main = paste0("PACF of ARIMA residuals - ", pollutant, " (MA)")
  )
  dev.off()
  ...
  kpss_file <- file.path(
    kpss_dir,
    paste0("KPSS_residuals_", pollutant, "_MA.txt")
  )
  sink(kpss_file)
  cat("KPSS test for ARIMA residuals -", pollutant, "(MA)\n\n")
  print(kpss_result)
  sink()
  ...
  w=which(abs(y_train)>5)
  y_train[w]=5
  v=which(abs(y_test)>5)
  y_test[v]=5
  ...
  # Reconstruction: sign(y) * sigma + ARIMA fitted mean
  ...
  p <- ggplot(df_plot, aes(x = date)) +
    ...
    labs(
      title = paste0("Predictions Based on ARIMA + MS-GARCH ", pollutant, " (MA)"),
      x = "Date",
      y = pollutant
    ) +
  ...
  pred_dir <- "/Users/rohan/Documents/Rohan/Fall 25/STATINF1/plots/predictions_MA"
  dir.create(pred_dir, recursive = TRUE, showWarnings = FALSE)
  ggsave(
    filename = file.path(
      pred_dir,
      paste0("Predictions_", pollutant, "_MA.png")
    ),
    plot   = p,
    width  = 10,
    height = 6,
  ...
  ###############################################################
  ## 8. EXTREME VALUE ANALYSIS (non-stationary GPD via RtsEva)
  ##    EVA on ACTUAL series using TsEvaNs
  ###############################################################
  ...
  # Take the GPD-based return levels (first element of ReturnLevels)
  ...
  eva_file <- file.path(
    eva_dir,
    paste0("EVA_RtsEva_GPD_", pollutant, "_MA.png")
  )
  ...
  # 9. Return everything you might want ------------------------------------
  list(
    pollutant      = pollutant,
    ...
    # EVA outputs (from RtsEva, non-stationary GPD on actuals)
...
k <- 3
fit_co_ma   <- fit_pollutant_msgarch_ma(ma_multi, "CO",  k)
fit_so2_ma  <- fit_pollutant_msgarch_ma(ma_multi, "SO2", k)
fit_no2_ma  <- fit_pollutant_msgarch_ma(ma_multi, "NO2", k)
fit_pm25_ma <- fit_pollutant_msgarch_ma(ma_multi, "PM25", k)
...
# ARIMA outputs:
fit_co_ma$arima_fit
fit_so2_ma$arima_fit
fit_no2_ma$arima_fit
fit_pm25_ma$arima_fit

# MS-GARCH outputs:
fit_co_ma$msgarch_fit
fit_so2_ma$msgarch_fit
fit_no2_ma$msgarch_fit
fit_pm25_ma$msgarch_fit

# RMSE In sample
fit_co_ma$rmse_insample
fit_so2_ma$rmse_insample
fit_no2_ma$rmse_insample
fit_pm25_ma$rmse_insample

# RMSE Out sample
fit_co_ma$rmse_outsample
fit_so2_ma$rmse_outsample
fit_no2_ma$rmse_outsample
fit_pm25_ma$rmse_outsample

# Return Levels 10 Year
fit_co_ma$return_level_10
fit_so2_ma$return_level_10
fit_no2_ma$return_level_10
fit_pm25_ma$return_level_10

# Return Levels 20 Year
fit_co_ma$return_level_20
fit_so2_ma$return_level_20
fit_no2_ma$return_level_20
fit_pm25_ma$return_level_20
